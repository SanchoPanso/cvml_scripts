...
class TubeGenerator:
    def __init__(self, 
        ...
        self.crop_images = {} # dict with keys - crop class name, values - list of images
        for crop_class_name in crop_class_names:
            ...
            if not os.path.exists(crop_class_dir):
                self.crop_images.update({crop_class_name: None})
                continue
            ...
            self.crop_images[crop_class_name] = img_list
        
        self.tube_images = os.listdir(self.tube_images_dir)
    
    def __call__(self, p=1.0, num_obj=10, **kwargs):
        ...
        for idx in range(num_obj):
            ...
            im, bbox = self.random_paste_img_to_img(tube_img, crop_img, labels)
            if bbox is None:
                continue
    ...
    def select_tube_image_and_labels(self) -> Tuple[np.ndarray, List[List[float]]]:
        img_path = random.choice(self.tube_images)
        name, ext = os.path.splitext(img_path)
        lbl_path = os.path.join(self.tube_labels_dir, name + '.npy')
    ...
    def select_crop_image(self, class_name: str) -> np.ndarray:
        img_path = random.choice(self.crop_images[class_name])
        img = np.load(img_path)
        return img
