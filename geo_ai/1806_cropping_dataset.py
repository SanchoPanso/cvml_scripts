...
def main():
    annot_paths = [
        r'C:\Users\HP\Downloads\Telegram Desktop\готовая разметка\3764-3766.json',
        r'C:\Users\HP\Downloads\Telegram Desktop\готовая разметка\3767.json',
        r'C:\Users\HP\Downloads\Telegram Desktop\готовая разметка\3768.json',
        r'C:\Users\HP\Downloads\Telegram Desktop\готовая разметка\3860.json',
        r'C:\Users\HP\Downloads\Telegram Desktop\готовая разметка\3861.json',
        r'C:\Users\HP\Downloads\Telegram Desktop\готовая разметка\07 CM_all.json',
    ]
    image_tiff_dir = r'D:\datasets\geo_ai\geoai_aerial\geoai_aerial_images_tiff'
    image_crop_dir = r'D:\datasets\geo_ai\geoai_aerial\geoai_aerial_images_cropped'
    
    size = (1000, 1000)
    isthing = [False, True, True, True, True, True, True, False, False, False]
    filtering_cls_id = 8
    
    # crop_images(image_tiff_dir, image_crop_dir, size)
    # annots = [cvml2.read_coco(path) for path in annot_paths]
    
    # cropped_annots = []
    # for an in annots:
    #     an1 = make_cropped_annotation(an, size, isthing)
    #     filter_mask(an1, image_crop_dir, filtering_cls_id)
    #     cropped_annots.append(an1)
    
    # common_annot = {}
    # common_annot['categories'] = cropped_annots[0]['categories']
    # common_annot['images'] = {}
    # for an in cropped_annots:
    #     common_annot['images'].update(an['images'])
        
    # with open('intermediate.json', 'w') as f:
    #     json.dump(common_annot, f)
    
    # with open('intermediate.json', 'r') as f:
    #     annot = json.load(f)
    
    # image_sources = cvml2.convert_paths_to_single_sources(glob.glob(image_crop_dir + r'\*'), lambda x: x)
    ...
    create_splitted_dataset(
        r'D:\datasets\geo_ai\geoai_aerial\geoai_aerial_18062023',
        r'D:\datasets\geo_ai\geoai_aerial\geoai_aerial_18062023',
        999*1024*1024,
    )
